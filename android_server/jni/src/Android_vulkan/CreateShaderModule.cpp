...
shaderc_shader_kind getShadercShaderType(VkShaderStageFlagBits type) {
  switch (type) {
    ...
    default:
      __android_log_assert("invalid VKShaderStageFlagBits", "tutorial06_texture", "type = %08x", type);
  }
...
VkResult buildShaderFromFile(android_app* appInfo, const char* filePath, VkShaderStageFlagBits type, VkDevice vkDevice, VkShaderModule* shaderOut) {
  ...
  shaderc_compilation_result_t spvShader = shaderc_compile_into_spv(compiler, glslShader.data(), glslShaderLen, getShadercShaderType(type), "shaderc_error", "main", nullptr);
