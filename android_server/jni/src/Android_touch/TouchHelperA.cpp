...
// ── Staging atomics: TypeA escribe aqui, hilo principal aplica a ImGui ──────
static volatile float  g_stg_x    = 0.f;
static volatile float  g_stg_y    = 0.f;
static volatile bool   g_stg_down = false;
        ...
        for (size_t j = 0; j < count; j++) {
            ...
            if (ie.code == SYN_REPORT) {
                if (Finger[i][latest].isDown) {
                    float sx = (scale_x > 0.001f) ? scale_x : 1.0f;
                    float sy = (scale_y > 0.001f) ? scale_y : 1.0f;
                    float x = (float)Finger[i][latest].x;
                    float y = (float)Finger[i][latest].y;
                    float xt = x / sx;
                    float yt = y / sy;
                    if (other_touch) {
                        switch (orientation) {
                            case 1:  x = xt;  y = yt;  break;
                            case 2:  y = yt;  x = screenHeight - xt;  break;
                            case 3:  x = screenHeight - xt;  y = screenWidth - yt;  break;
                            default: y = xt;  x = screenHeight - yt;  break;
                        }
                    } else {
                        switch (orientation) {
                            case 1:  x = yt;  y = screenHeight - xt;  break;
                            case 2:  x = screenHeight - xt;  y = screenWidth - yt;  break;
                            case 3:  y = xt;  x = screenWidth - yt;  break;
                            default: x = xt;  y = yt;  break;
                        }
                    }
                    if (x < 0.f) x = 0.f;
                    if (y < 0.f) y = 0.f;
                    // escribe en staging — el hilo principal aplica en el proximo frame
                    g_stg_x    = x;
                    g_stg_y    = y;
                    g_stg_down = true;
                } else {
...
// Llamar desde el hilo principal justo ANTES de ImGui::NewFrame()
// Aplica el estado del touch a ImGui IO sin races entre hilos
void Touch_ApplyToImGui() {
    ImGuiIO& io = ImGui::GetIO();
    if (g_stg_down) {
        io.MousePos    = { g_stg_x, g_stg_y };
        io.MouseDown[0] = true;
    } else {
